    ...
    <div class="container flex items-center justify-between md:justify-end h-full px-6 mx-auto text-purple-600">
        ...
        <ul class="flex items-center flex-shrink-0 space-x-6">
            <li class="relative" x-data="{ isProfileMenuOpen: false }">
                <button
                    class="flex items-center gap-2 align-middle rounded-lg focus:shadow-outline-purple focus:outline-none"
                    x-on:click="isProfileMenuOpen = !isProfileMenuOpen" x-on:keydown.escape="isProfileMenuOpen = false"
                    ...
                    $initial = strtoupper(substr($name, 0, 1)); // Ambil huruf pertama nama
                    ?>

                    <?php if (!empty($photo)): ?>
                        <div class="relative w-8 h-8">
                            <img class="object-cover w-8 h-8 rounded-full shadow-md"
                                src="<?= base_url('uploads/photos/' . $photo) ?>" alt="Foto Profil" loading="lazy"
                    ...
                    <?php else: ?>
                        <div
                            class="flex items-center justify-center w-8 h-8 rounded-full bg-blue-500 text-white text-sm font-semibold">
                            <?= esc($initial) ?>
                        </div>
                    <?php endif; ?>
                ...
                <ul x-show="isProfileMenuOpen" x-transition:enter="transition ease-out duration-150"
                    ...
                    <li class="flex">
                        <a class="inline-flex items-center w-full px-2 py-1 text-sm font-semibold transition-colors duration-150 rounded-md hover:bg-gray-100 hover:text-gray-800"
                            href="<?= site_url('akun') ?>">
                            <svg class="w-4 h-4 mr-3" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none"

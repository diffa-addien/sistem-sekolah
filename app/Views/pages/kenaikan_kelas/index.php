...
<?= $this->section('title') ?>Manajemen Kenaikan Kelas & Kelulusan<?= $this->endSection() ?>
...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="bg-white rounded-xl shadow-lg p-6 mb-8">
        <form action="<?= site_url('admin/kenaikan-kelas') ?>" method="get">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                <div>
                    <label for="from_year_id" class="block text-sm font-medium text-gray-700 mb-1">Dari Tahun Ajaran</label>
                    <select name="from_year_id" id="from_year_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg" required>
    ...
    <form action="<?= site_url('admin/kenaikan-kelas/proses') ?>" method="post">
            ...
            <div class="class-card bg-white rounded-xl shadow-lg p-6 mb-6">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-gray-200 pb-3 mb-4">
                    ...
                    <div class="w-full sm:w-auto mt-3 sm:mt-0">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Aksi Massal:</label>
                        <select class="bulk-action-select w-full sm:w-64 px-3 py-2 border border-gray-300 rounded-lg">
                            <option value="tinggal_kelas">-- Terapkan ke Semua: Tinggal Kelas --</option>
                            <optgroup label="Naikkan ke Kelas">
                            ...
                            <optgroup label="Aksi Lain">
                                ...
                                <option value="keluar">Keluarkan Semua</option>
                            </optgroup>
                        </select>
                    </div>
                </div>
                <div class="overflow-x-auto">
                    <table class="w-full text-left">
                        <thead><tr class="text-xs font-semibold text-gray-500 uppercase border-b"><th class="py-3 px-4">Nama Siswa</th><th class="py-3 px-4">Aksi Individual</th></tr></thead>
                        <tbody class="divide-y divide-gray-200">
                            <?php foreach ($class['students'] as $student) : ?>
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="py-3 px-4">
                                        <select name="actions[<?= $student['id'] ?>]" class="student-action-select w-full px-3 py-2 border border-gray-300 rounded-lg">
        ...
        <div class="flex justify-end mt-6">
            <button type="submit" class="px-6 py-3 bg-red-600 text-white rounded-lg hover:bg-red-700" onclick="return confirm('PERINGATAN: Aksi ini akan mengubah status pendaftaran siswa secara permanen. Lanjutkan?');">

    ...
    <div class="w-full overflow-hidden rounded-2xl border border-gray-300">
        <div class="w-full overflow-x-auto">
            <table class="w-full whitespace-no-wrap">
                <thead><tr class="text-xs font-semibold text-center text-gray-500 uppercase border-b bg-gray-50"><th class="px-4 py-3 text-left sticky left-0 bg-gray-100 z-10">Nama Siswa</th><?php foreach ($dateHeaders as $date) : ?><th class="px-3 py-3"><?= date('d', strtotime($date)) ?></th><?php endforeach; ?></tr></thead>
                <tbody class="bg-white divide-y">
                    <?php if (!empty($reportData)): foreach ($reportData as $nis => $data) : ?>
                    <tr class="text-gray-700">
                        <td class="px-4 py-3 text-sm font-semibold sticky left-0 bg-white"><a href="<?= site_url('admin/laporan/siswa/' . $data['student_id']) ?>" title="Lihat Laporan" class="hover:underline text-sky-600"><?= esc($data['full_name']) ?></a></td>
                    ...
                    <tr><td colspan="<?= count($dateHeaders) + 1 ?>" class="text-center py-4 text-gray-500">Tidak ada data kehadiran.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
